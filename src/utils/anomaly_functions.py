...
def keep_top_percent(confidence: torch.Tensor, percent: float, binarize=False):
    ...
    assert (
        0 < percent < 1
    ), f"Error: Percentage must be between 0 and 1 (excluded), but got {percent}"
...
def binarize_mse_af(actual, estimate, threshold=0.5):
...
def confidence_mse_af(actual, estimate, confidence):
    ...
    return (
        torch.mean(confidence * (estimate - actual) ** 2, dim=[1, 2, 3])
        .detach()
        .cpu()
        .numpy()
    )
...
def binarize_confidence_mse_af(actual, estimate, confidence, threshold=0.5):
